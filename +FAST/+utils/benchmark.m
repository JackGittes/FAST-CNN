...
function benchmark()
    if gpuDeviceCount > 0
        fprintf('Benchmark will begin with GPU enabled.\n');
        fprintf('Single core CPU test begins on MobileNet v1@FXP16.8.\n');
        res_single_core = benchmark_exec('SingleCore');
        fprintf('Multi-core CPU test begins on MobileNet v1@FXP16.8.\n');
        res_multi_core = benchmark_exec('MultiCore');
        fprintf('GPU test begins on MobileNet@FXP16.8.\n');
        res_gpu = benchmark_exec('GPU');
    else
        fprintf('No GPU found, benchmark will begin with CPU only.\n');
        fprintf('Single core CPU test begins on MobileNet v1@FXP16.8.\n');
        res_single_core = benchmark_exec('SingleCore');
        fprintf('Multi-core CPU test begins on MobileNet v1@FXP16.8.\n');
        res_multi_core = benchmark_exec('MultiCore');
...
function res = benchmark_exec(type)
wordlen =16;
fraclen =8;
